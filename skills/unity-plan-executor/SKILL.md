...
- **Specialist Coordination**: Dynamically load the correct specialized skills (e.g., `unity-coder`, `unity-tech-artist`, `flatbuffers`) based on the task type.
...
2.  **Specialist Selection**:
    ...
    - **Load Skill**: `unity-coder` (for logic), `unity-tech-artist` (for shaders/tools), `flatbuffers` (for data), etc.
...
4.  **Self-Correction & Refinement**:
    ...
    - If an implementation detail from the guide fails (e.g., due to codebase changes), use `unity-code-investigator` to find a fix.
...
- **Specialist Persona**: When using a specialized skill (like `unity-coder`), fully adopt its best practices and patterns.
...
- **If a task requires cross-disciplinary work** -> Load both `unity-coder` and `unity-tech-artist` in sequence to finish the task.
